...
def main():
    ...
    if platform.system() == "Windows":
        config["output"] = "bin/game.exe"
    else:
        config["output"] = "bin/game"
...
def compile(config):
    ...
    if platform.system() == "Windows" and config["use_compiler"] == "msvc":
        MSVC_COMPILE_OPTIONS    = "/std:c++17 /nologo /EHsc /MTd /TP /wd4129"
        MSVC_LINK_OPTIONS       = "/nologo /ignore:4099 Advapi32.lib shell32.lib"
        MSVC_INCLUDE_DIRS       = "/Isrc/include /Ilibs/tracy-0.10/public /Ilibs/GLEW/include /Ilibs/GLFW/include /Ilibs/reactphysics3d/include /Ilibs/include"
        MSVC_LIB_DIRS       = "/LIBPATH:libs/GLFW/lib /LIBPATH:libs/GLEW/lib /LIBPATH:libs/reactphysics3d/lib"
        MSVC_DEFINITIONS        = "/DOS_WINDOWS /DENGONE_PHYSICS /DENGONE_LOGGER /DENGONE_OPENGL"
        
        MSVC_LIBS = "opengl32.lib Ws2_32.lib shell32.lib winmm.lib gdi32.lib user32.lib rp3d.lib"

        if enabled("hotreloading"):
            MSVC_COMPILE_OPTIONS += " /DGLFW_DLL"
            MSVC_COMPILE_OPTIONS += " /DLIVE_EDITING"
        else:
            MSVC_COMPILE_OPTIONS += " /DGLEW_STATIC"
        ...
        if len(modified_files) == 0 and os.path.exists(config["output"]):
            ...
            if enabled("unity_build"):
                ...
                if enabled("hotreloading"):
                    err = cmd("link /DLL "+MSVC_LINK_OPTIONS+" "+objs_str +" /OUT:"+"bin/prounk.dll")
                    err = cmd("link "+MSVC_LINK_OPTIONS+" "+MSVC_LIB_DIRS+" "+objs_str+" /SUBSYSTEM:CONSOLE "+"bin/prounk.dll /OUT:"+config["output"])
                else:
                    extra_libs = "rp3d.lib glfw3_mt.lib glew32s.lib"
                    err = cmd("link "+MSVC_LINK_OPTIONS+" "+MSVC_LIBS+" "+extra_libs+" "+MSVC_LIB_DIRS+" /SUBSYSTEM:CONSOLE "+objs_str +" /OUT:"+config["output"])
                compile_success = err == 0
