...
void Text::DrawString(float alpha, bool f) {
	// Dynamic
	float actualHeight = renderer::AlterH(charHeight);
	if (!staticHeight) {
		actualHeight = ((*elemW / 2 * renderer::Width()) / PixelWidth(true)) * renderer::AlterH(64);
	...
	renderer::GuiTransform(xpos, ypos);
	renderer::GuiColor(r, g, b, a*alpha);
	if (f){
		renderer::DrawText(font, text, center, actualHeight, atChar);
	} else {
		renderer::DrawText(font, text, center, actualHeight, -1);
	}
	/*
	if (f) {
