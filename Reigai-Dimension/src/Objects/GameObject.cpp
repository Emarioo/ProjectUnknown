...
glm::vec3 GameObject::WillCollide(GameObject* o1,float delta) {
	std::vector<ColliderComponent*> list0=GetColliders();
	std::vector<ColliderComponent*> list1=o1->GetColliders();
	glm::vec3 out = velocity;

	std::vector<ColliderComponent*>* plist0 = &list0;
	std::vector<ColliderComponent*>* plist1 = &list1;
	...
	while(times++<2){
		...
		for (ColliderComponent* c0 : *plist0) {
			if (c0->coll == nullptr)
				continue;
			for (ColliderComponent* c1 : *plist1) {
				if (c1->coll == nullptr)
					continue;
				bool hasHit = true;
				int limit = 0;
...
void GameObject::PreComponents() {
	
}
std::vector<ColliderComponent*> GameObject::GetColliders() {
