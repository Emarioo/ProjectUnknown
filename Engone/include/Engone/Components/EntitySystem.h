...
namespace engone {
	...
	public:
		//EntityStack() = default;
		EntityStack(ComponentMask mask) : m_mask(mask) {
	...
	public:
		...
		StackCollection(ComponentMask mask) : m_mask(mask) {}
	...
	*/
	...
	public:
		...
		// will crash if next hasn't been called before
		Entity getEntity() {
			return {collection->m_mask,collection->stacks[stackIndex]->componentSizes,
