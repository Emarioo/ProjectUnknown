...
namespace engone {
	...
	*/
	...
	void LoadMesh(Mesh* data, const std::string& path_) {
		...
		if (dAll || dGeneral) bug::outs < "Mesh =" < path_<"\n";
		...
		*/
		...
		data->container.Init(false, vertexOut, (uniqueVertex.size() / uvStride) * vStride, triangleOut, triangleCount * 3);
	...
	void LoadAnimation(Animation* data, const std::string& path_) {
		...
		if (dAll || dGeneral) bug::out < "Animation  " < path_< bug::end;
		...
		for (int i = 0; i < objects; i++) {
			...
			error(Read<unsigned short>(&index));
			...
			if (dAll || dFrames) {
				/*std::string order[]{"PX","PY","PZ","RX","RY","RZ","SX","SY","SZ","QX","QY","QZ","QW"};
				std::string printing = "";
				for (int n = 0; n < 13;n++) {
					if (curveB[n])
						printing += ","+order[n];
				...
				if (printing.length() > 0)
					printing = printing.substr(1);*/
				std::cout << "Object " << i << " Parent: " <<index << std::endl;
			}

			data->objects[index]=Channels();
			Channels* channels = &data->objects[index];
			...
			for (ChannelType cha = PosX; cha < 13; cha =(ChannelType)(cha +1)) {
				if (curveB[cha]) {
					...
					if (dAll || dFrames) std::cout << " Curve " << curve_order[cha] << " " << (int)keys << std::endl;
					...
					for (int k = 0; k < keys; k++) {
						...
						error(Read<PolationType>(&polation)); // 1 byte
	...
	void LoadArmature(Armature* data, const std::string& path_) {
		...
		for (int i = 0; i < boneCount; i++) {
			Bone b;
			error(Read<unsigned short>(&b.parent));
