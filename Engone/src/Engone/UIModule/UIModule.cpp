...
#include "Engone/UIModule/VertexShader.glsl"

#include "Engone/Window.h"

#define GL_CHECK_UI()  {int err = glGetError();if(err) {printf("UIMod GLError: %d %s\n",err,(const char*)glewGetErrorString(err));DebugBreak();}}
...
void UIModule::drawBatch(int& dataIndex){
    if(dataIndex!=BATCH_BOX_LIMIT){
        ZeroMemory(vertexData+4*dataIndex,(BATCH_BOX_LIMIT-dataIndex)*sizeof(UIVertex)*4);
    }
    ...
    while(true){
        ...
        if(rawType == UI_ELEMENT_TEXT){
            ...
            while(true){
                ...
                if(chrIndex==length)
                    break;
                unsigned char chr = t->text[chrIndex];
                chrIndex++;
    ...
    for(int i=0;i<total;i++){
        ...
        if(a)
            *a = {10.f+bw*(i%mw),
            60.f+bh*(i/mw),
            bh,
            texts[(2*i+1)%(sizeof(texts)/sizeof(const char*))],
...
void TestUIMaintain(UIModule& ui){
    ...
    for(int i=0;i<32;i++){
        ...
        // p = {i*10.f+10,200,8,8,{i/32.f,1,1,1}};
        if(p.x==0)
            p = {i*10.f+10,200,15,"Sup",{i/32.f,1-i/32.f,i*i/(32.f*32.f)-2*i/32.f,1}};
            // p = {i*10.f+10,200,8,8,{i/32.f,1-i/32.f,i*i/(32.f*32.f)-2*i/32.f,1}};
        else{
