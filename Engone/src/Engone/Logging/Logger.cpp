...
namespace engone {
	...
	void Logger::print(char* str, int len) {
		...
		if (info.masterColor == log::NO_COLOR)
			SetConsoleTextAttribute(m_consoleHandle, info.color);
		else
			SetConsoleTextAttribute(m_consoleHandle, info.masterColor);
		printf("%s", str);
		m_printMutex.unlock();
