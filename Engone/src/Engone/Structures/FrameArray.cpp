...
namespace engone {
	FrameArray::FrameArray(uint32 typeSize, uint32 valuesPerFrame, uint32 allocType) :
	...
	FrameArray::Frame::Frame(uint32 allocType) : memory{1,allocType} {}

	// Returns -1 if something failed
	uint32 FrameArray::add(void* value, void** outPtr) {
	...
	void* FrameArray::get(uint32 index) {
	...
	void FrameArray::remove(uint32 index) {
		uint32 frameIndex = index / m_valuesPerFrame;
		uint32 valueIndex = index % m_valuesPerFrame;
	...
	uint32 FrameArray::getMemoryUsage() {
	...
	// 8*max / (1+8*C)
	// Memory<char> memory{};
	void* FrameArray::Frame::getValue(uint32 index, uint32 typeSize, uint32 vpf) {
		//return (Value*)(memory.data+ vpf/8+index*sizeof(Value));
		return (void*)((char*)memory.data + vpf + index * typeSize);
	}
	bool FrameArray::Frame::getBool(uint32 index) {
		//uint32 i = index / 8;
		//uint32 j = index % 8;
	...
	void FrameArray::Frame::setBool(uint32 index, bool yes) {
		//uint32 i = index / 8;
		//uint32 j = index % 8;
		//if (yes) {
	...
	void FrameArrayTest() {
		...
		Apple tmp{ 25,15,92 };
		uint32 id_a = arr.add(&tmp);
		tmp = { 25,15,92 };
		uint32 id_b = arr.add(&tmp);
