...
namespace prounk {
	...
	u32 InventoryRegistry::createInventory(Inventory** outPtr) {
		Inventory tmp{};
		return m_inventories.add(&tmp,(void**)outPtr) + 1;
	}
	Inventory* InventoryRegistry::getInventory(u32 dataIndex) {
		if (dataIndex == 0) return nullptr;
		return (Inventory*)m_inventories.get(dataIndex - 1);
	}
	void InventoryRegistry::destroyInventory(u32 dataIndex) {
