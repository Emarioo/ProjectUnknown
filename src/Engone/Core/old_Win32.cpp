...
#define TO_INTERNAL(X) (void*)((u64)X+1)
#define TO_HANDLE(X) (HANDLE)((u64)X-1)
	...
	DirectoryIterator* DirectoryIteratorCreate(const std::string& path, DirectoryIteratorData* result){		
		...
		return (DirectoryIterator*)((u64)handle+1); // +1 in case 0 is a valid handle. Could not find anything about this
	}
	bool DirectoryIteratorNext(DirectoryIterator* iterator, DirectoryIteratorData* result){
		WIN32_FIND_DATAA data;
		while(true){
			BOOL success = FindNextFileA((HANDLE)((u64)iterator-1),&data);
			if(!success){
				DWORD err = GetLastError();
				if(err == ERROR_NO_MORE_FILES){
	...
	void DirectoryIteratorDestroy(DirectoryIterator* iterator){
		BOOL success = FindClose((HANDLE)((u64)iterator-1));
		if(!success){
			DWORD err = GetLastError();
			PL_PRINTF("[WinError %lu] Error closing '%llu'\n",err,(u64)iterator);
		}
	...
	bool FileCopy(const std::string& src, const std::string& dst){
		bool yes = CopyFileA(src.c_str(),dst.c_str(), false);
		if(!yes){
			DWORD err = GetLastError();
			if(err == ERROR_FILE_NOT_FOUND) {
				return false;	
...
#define DEBUG_PLATFORM_ERROR(x) x
	...
	u32 RunMonitor(void* arg){
		static const u32 INITIAL_SIZE = 5 * (sizeof(FILE_NOTIFY_INFORMATION) + 500);
	...
	bool FileMonitor::check(const std::string& path, void(*callback)(const std::string&, u32), u32 flags) {
	// bool FileMonitor::check(const std::string& path, std::function<void(const std::string&, u32)> callback, u32 flags) {
