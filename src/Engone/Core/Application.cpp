...
//#endif
		...
		EngineWorld* world = (EngineWorld*)GetGameMemory().allocate(sizeof(EngineWorld));
	...
	void Application::setFPS(double fps, u32 windowIndex) {
		getWindow(windowIndex)->setFPS(fps);
	}
	double Application::getFPS(u32 windowIndex) {
		return getWindow(windowIndex)->getFPS();
	}
	double Application::getRealFPS(u32 windowIndex) {
