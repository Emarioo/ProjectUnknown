...
// needs global engone -> active ui renderer
...
namespace engone {
			...
			if (type == 'M') {
				...
				if (!shader->getError()) {
					...
					for (auto& pair : normalObjects) {
						...
						u32 batchSize = UIRenderer::INSTANCE_BATCH;
						while (remaining > 0) {
	...
	void UIRenderer::setActiveUIRenderer() {
		...
		GetGlobalEngone()->activeUIRenderer = this;
		
	}
	UIRenderer* GetActiveUIRenderer() {
		// return s_activeUIRenderer;
		return GetGlobalEngone()->activeUIRenderer;
	}
	UIRenderer::~UIRenderer() {
		// if (s_activeUIRenderer == this)
		// 	s_activeUIRenderer = nullptr;
		if (GetGlobalEngone()->activeUIRenderer = this)
			GetGlobalEngone()->activeUIRenderer = nullptr;
	}
}