...
namespace engone {
	...
	HashMap::HashMap(u32 typeSize, u32 allocType, u32 keymapSize) : 
		m_typeSize(typeSize), m_allocType(allocType), m_keymapSize(keymapSize), 
		m_keymap(sizeof(Chain)+typeSize,allocType), m_chainData(sizeof(Chain)+typeSize,allocType), m_emptySpots(sizeof(u32),allocType) {
	...
	u32 HashMap::getMemoryUsage() {
		return m_keymap.max*m_keymap.getTypeSize() + m_chainData.max * m_chainData.getTypeSize()+m_emptySpots.size()*sizeof(u32);
	}
	u32 HashMap::hash(u32 key) {
		// will crash if keymap.max is 0.
		u32 index = key % m_keymap.max; // yes, you should laugh
		return index;
	}
	u32 HashMap::createChain() {
		u32 indexNext=0;
	...
	bool HashMap::insert(u32 key, void* value) {
		...
		while (true) {
			printf("Loop %u %u %u\n", chain->key, *(u32*)chain->value, chain->indexNext);
			if (!chain->used) {
				chain->used = true;
				chain->key = key;
				//chain->value = value;
				...
				printf("v3 %u %u\n", newChain->key, *(u32*)newChain->value);
	...
	HashMap::Chain* HashMap::getDChain(u32 index) {
		return (Chain*)((char*)m_chainData.data + index * m_chainData.getTypeSize());
	}
	HashMap::Chain* HashMap::getKChain(u32 index) {
	...
	void HashMapTestCase() {
		HashMap map(sizeof(u32),ALLOC_TYPE_HEAP, 20);
		HashMap map2(sizeof(u32), ALLOC_TYPE_HEAP, 20);
