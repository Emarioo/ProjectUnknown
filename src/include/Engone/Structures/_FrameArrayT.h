...
namespace engone {
	...
	public:
		...
		FrameArray(u32 valuesPerFrame, int allocType) : m_valuesPerFrame(valuesPerFrame), m_allocType(allocType), m_frames{sizeof(Frame),allocType) {
	...
	private:
		struct Frame {
			...
			Memory memory{1};
			int count = 0;
			Value* getValue(u32 index, u32 vpf) {
