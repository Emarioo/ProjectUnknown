...
def DecideColliderType(original):
    data=original.data
...
def WriteCollider(path,original):
    ...
    if colliderType=="CUBE":
        ...
        if not len(object.data.vertices)==width*height:
            Logging('WARNING',"Height map size was miscalculated!")
        else:
            ...
            if not median.x==0 or not median.z == 0:
                ...
                Logging('WARNING',"Height map is not centered!")
            ...
            scaleY = original.scale.z
            scaleZ = original.dimensions.y/(width-1)
...
class MExportOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if projectPath=="":
            ...
            projectPath = me.projectPath if projectPath[-1]=='\\' else projectPath+'\\'
