...
namespace engine {
	...
	void LoadGameData() {// ALL game assets should be added here(Not interface stuff)
		...
		//renderer::AddTexture("magicstaff_staff");
		//renderer::AddTexture("magicstaff_fireball");
...
//		dManager::AddMesh("RLArm");
		...
		//dManager::AddAnim("TestAnim2");// ->defaultSpeed = 12;

		//dManager::AddBone("TestBone");
		//dManager::AddMesh("TestMesh");
		/*
		AnimData* modif = dManager::AddAnim("Modified");
		modif->hasError = false;
		modif->frameStart = 0;
		modif->frameEnd = 90;
		...
		FCurve curve;
		curve.Add('B',0,0);
		curve.Add('B',30,glm::radians(45.f));
		curve.Add('B',60,glm::radians(45.f));
		curve.Add('B',90,0);
		FCurve curve2;
		curve2.Add('B', 30, 0);
		curve2.Add('B', 60, glm::radians(45.f));
		curve2.Add('B', 90, 0);
		curves.Add(3,curve);
		curves2.Add(4,curve2);
		modif->Add(0,curves);
		modif->Add(1,curves2);
		*/
...
//		dManager::AddColl("VertexColor");
	...
	void Init() {
		...
		SetGameState(GameState::play);// Temporary
		...
		*/
		...
		//oManager::AddObject(new OTemplate(0, 0, -5));

		//oManager::AddObject(new MagicStaff(0, 0, -5));

		//oManager::AddObject(new Goblin(0, 0, 0));
	...
	void Render() {
		if (dManager::GetGameState() == GameState::menu) {
			...
			if (renderer::HasFocus() && !iManager::IsPauseMode()||!pauseRender) {
				//bug::out < "Rend " < timeOfDay < bug::end;
				timeOfDay += daySpeed;
				if (timeOfDay > 1 || timeOfDay < 0)
					daySpeed *= -1;
				glClearColor(0.2f * timeOfDay, 0.4f * timeOfDay, 0.9f * timeOfDay, 1);
					...
					for (int i = 0; i < o->renderComponent.meshes.size(); i++) {
						...
						if (mesh != nullptr) {
							if (mesh->material == MaterialType::ColorMat) {
								renderer::DrawMesh(mesh, o->renderComponent.matrices[i]);
							}
						} else {
				...
				renderer::BindShader(MaterialType::ColorWeightMat);
				for (GameObject* o : *oManager::GetObjects()) {
					if (o->renderComponent.bone.enabled) {
						...
						//bug::out < "Load mats" < bug::end;
						for (int i = 0; i < count; i++) {
							//bug::out < "e"<bug::end;
							renderer::GetShader(MaterialType::ColorWeightMat)->SetUniformMat4fv("uBoneTransforms[" + std::to_string(i) + "]", mats[i]);
						}
					}/* else {
						continue;
					}*/
					for (int i = 0; i < o->renderComponent.meshes.size(); i++) {
						MeshData* mesh = o->renderComponent.meshes[i];
						if (mesh != nullptr) {
							if (mesh->material == MaterialType::ColorWeightMat) {
								//bug::out < "r"<bug::end;
								renderer::DrawMesh(mesh, o->renderComponent.matrices[i]);// Could be causing problems by not giving it an empty matrix
							}
						}
					}
				}
