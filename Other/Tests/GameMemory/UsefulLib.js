...
class Color {
	...
	static FromHex(hex){
		if(hex.length==3){
			return new Color(hex.indexOf(h[1])*17,hex.indexOf(h[2])*17,hex.indexOf(h[3])*17);
		}else{
			return new Color(hex.indexOf(h[1])*16+hex.indexOf(h[2]),hex.indexOf(h[3])*16+hex.indexOf(h[4]),hex.indexOf(h[5])*16+hex.indexOf(h[6]));
		}	
	}
	darken(val){
		this.r-=val;
